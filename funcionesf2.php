<?php
      ...
      foreach($listaCarreras as $carrera){
        ...
        for($i = 1; $i <= count($posicionesCarrera); $i++){
          if(strpos($nombre, $posicionesCarrera[$i]) != false || $nombre == $posicionesCarrera[$i]){
            if($tipo == 'piloto'){
              $posicion = $i;
              $informacion['puntos'] = $informacion['puntos'] + calcularPuntos($temporada, $posicion, $carrera['tipo']);
              $i = 1000;
            }
            else{
              $posicion = $i;
        ...
        if(($carrera['tipo'] == "Feature") && (hizoLaPole($nombre, $carrera, $tipo))) $informacion['poles']++;
    ...
    function calcularPuntos($temporada, $posicion, $tipoCarrera){
      if($tipoCarrera == "Feature"){
        ...
        if($temporada >= 2017){
          ...
          if($posicion == 5) return 6;
          if($posicion == 6) return 4;
          if($posicion == 7) return 2;
    ...
    function calcularPuntosPorPole($poles, $temporada, $tipoCarrera){
      if(($temporada >= 2017) && ($tipoCarrera == "Feature")) return $poles * 4;
      return 0;
    }
